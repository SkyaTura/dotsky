...
def get_battery_status():
    ...
    if ' discharging;' in pmset_output:
        return 'discharging'
    if ' charging;' in pmset_output:
        return 'charging'
    if ' charged;' in pmset_output:
        return 'charged'

def get_battery_level():
    battery_percentage = subprocess.check_output("pmset -g batt | grep -Eo '\d+%' | cut -d% -f1", shell=True).decode("utf-8").strip()
